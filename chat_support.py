...
def create_new_chat(directory, emb_option, llm_option, storage_context,
    ...
    # Set up LLM
    LLM_model = get_LLM(llm_option=llm_option, env_vars=os.environ)
    ...
    # Save the selected options to a config file
...
def load_prior_index(STORAGE_DIR, DEBUG=False):
    config_path = os.path.join(STORAGE_DIR, "config.json")
    ...
    if os.path.exists(config_path):
        with open(config_path, "r") as f:
            ...
            f.close()
        print('Embedded option: %s'%(st.session_state.selected_emb_option))
        print('LLM option: %s'%(st.session_state.selected_llm_option))
    else:
        print('Configuration file not found. Using default embedding and LLM options.')
    ...
    embedding_model = get_embedding(emb_option=st.session_state.selected_emb_option)
    Settings.embed_model = embedding_model
    Settings.llm = get_LLM(llm_option=st.session_state.selected_llm_option, env_vars=os.environ)
...
def load_user_documents(directory, emb_option, llm_option,
    ...
    try:
        ...
        load_prior_index(STORAGE_DIR, DEBUG=DEBUG)
